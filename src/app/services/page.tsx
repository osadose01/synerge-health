...
export const metadata = {
  ...
  description: "Explore Synerge Health's 4 core service pillars: Strategic Guidance, Operational Support, Funding, and Market Launch.",
};
        ...
        <section className="bg-white">
          {SERVICES.map((service, i) => (
            <div
              key={service.id}
              id={service.id}
              className={`py-24 md:py-32 border-t border-slate-100 scroll-mt-32 ${i % 2 === 1 ? "bg-slate-50" : "bg-white"}`}
            >
              <div className="container mx-auto px-8 md:px-16">
                <div className="grid md:grid-cols-[1fr_1.6fr] gap-24 items-start">
                  <p className="label-mono">{service.num}</p>
                  <div>
                    <h2 className="font-display font-bold text-3xl md:text-4xl text-slate-900 leading-[1.1] mb-8">
                      {service.title}
                    </h2>
                    <p className="text-base text-slate-500 leading-[1.8] max-w-lg mb-12">
                      {service.summary}
                    </p>
