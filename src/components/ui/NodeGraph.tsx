...
export function NodeGraph() {
  ...
  return (
    <div className="relative w-full flex flex-col items-center">
      ...
      {/* ── Mobile card row (shown < md) ──────────────────────────────── */}
      <div className="mt-10 md:hidden w-full overflow-x-auto flex gap-4 pb-4 snap-x snap-mandatory">
        {FOCUS_AREAS.map((node) => (
          <div
            key={node.id}
            className="snap-center shrink-0 w-56 rounded-xl border border-[rgba(43,224,176,0.15)] bg-[#0D1815] p-4"
          >
            <div className="font-mono text-[10px] tracking-[0.18em] uppercase text-[#2BE0B0] mb-2">
              {node.label}
            </div>
            <p className="text-xs text-[#8FA39A] leading-relaxed">{node.desc}</p>
          </div>
        ))}
      </div>
    </div>
